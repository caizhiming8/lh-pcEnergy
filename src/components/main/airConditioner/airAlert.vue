    ...
    <div id="makeHot">
        <div class="setting-main" ref="settingMain">
            <div class="setting-tree" ref="settingTree">
                <el-tree accordion :data="data" :props="defaultProps"  @node-click="handleNodeClick" highlight-current>
            ...
            <div class="setting-table">
                ...
                <div class="placeTable">
                    <el-table :data="tableData" style="margin-top: 20px" :row-style="{height:0}" :cell-style="{padding:'8px 0'}"   :header-cell-style="{background:'#F5F7FA',color:'#606266', height: '0px', padding: '15px 0'}"  v-loading = "loading">
                        ...
                        <el-table-column label="操作" align='center' width="180">
                            <template slot-scope="scope">
                                <el-button @click="handleClick(scope.row, scope.$index)" size="mini" type="success">修改</el-button>
                                <el-button @click="deletes(scope.row, scope.$index)" size="mini" type="danger">删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                </div>
...
export default {
    ...
    methods: {
        ...
        makeSureDelete() {
            ...
            this.$axios.post(`BMPlatServers/common/deleteDeviceThreshold`, this.$qs.stringify(param)).then((res)=>{
                if(res.data.success) {
                    ...
                    this.initDataUpdata(this.deleteDevideId, this.row.deviceType);
                    this.deviceAttribute();
                }else{
                    this.$message.warning({
                        message: res.data.resultMessage,
        ...
        // 表格初始化数据
        initTabelData() {
            this.$axios.post(`BMPlatServers/common/queryDeviceThreshold`).then((response) => {
				if(response.data.success){
					this.tableData = response.data.result;
				}else{
                    this.$message.warning({
        ...
        initDom() {
            let Height = this.$refs.settingMain.offsetHeight;
            this.$refs.settingTree.style.height = Height + 'px';
        },
