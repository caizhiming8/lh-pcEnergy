<template>
	<div id="publicRation">
		<div class="publicRation">
			<div class="ratingTree">
				<v-ratingTree @sonData="sonDatas"></v-ratingTree>
			</div>
			<div class="topSelfTube">
				...
				<div class="top">
					<div class="top-total">
						<span class="dinges">公共分摊定额配置:      {{selfTube}}</span>
					</div>
					<div class="top-points">
						...
						<el-button type="primary" @click="allocation" size="small"  class="allot" v-if="this.control == 1">分配</el-button>
					</div>
				</div>
			...
			<div class="center">
				<span class="table-center">公共分摊房间已分配定额</span>
				<el-table tooltip-effect="light" :data="tableData" stripe style="width: 100%; margin-top: 20px" :row-style="{height:0}" :cell-style="{padding:'8px'}" :header-cell-style="{background:'#F5F7FA',color:'#606266', height: '0px', padding: '5px'}">
					...
					<el-table-column :show-overflow-tooltip="true" prop="consum" label="公共总能额" align="center"></el-table-column>
					<el-table-column :show-overflow-tooltip="true" prop="1" label="公共特殊用电" align="center"></el-table-column>
					<el-table-column :show-overflow-tooltip="true" prop="4" label="公共器械用电" align="center"></el-table-column>
					<el-table-column :show-overflow-tooltip="true" prop="3" label="公共动力用电" align="center"></el-table-column>
					<el-table-column :show-overflow-tooltip="true" prop="2" label="公共空调用电" align="center"></el-table-column>
				</el-table>
			</div>
			<el-dialog title=公共分摊房间分配定额 :visible.sync="maleData" width="60%">
				<div class="el-form">
...
<script>
import ratingTree from './ratingTree'
export default{
	data(){
		return{
			selfTube: 0,
	...
	methods:{
		initData(){
			...
			console.log(data)
			this.$axios.post(`BMPlatServers/consum/queryggroomquota`, this.$qs.stringify(data)).then((res)=>{
				console.log(res)
				if(res.data.success){
					this.tableData = []
		...
		sonDatas(datas){
			...
			this.id = datas.id
			if(this.buildingId != ''&&this.floorId != ''&&this.buildingId != null&&this.floorId != null){
				this.initData()
			}else{
				this.tableData = []
				this.special = 0
	...
	    },
		...
		confirm(){
			...
			console.log(data)
			this.$axios.post(`BMPlatServers/consum/setggquota`, this.$qs.stringify(data)).then((res)=>{
				console.log(res)
				this.initData()
			})
			this.maleData = false
