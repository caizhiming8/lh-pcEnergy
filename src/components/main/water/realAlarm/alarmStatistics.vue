...
export default {
    ...
    methods: {
		...
		//查询各等级的报警数量
		this.$axios.post(`BMPlatServers/warn/queryWarningDegree`,this.$qs.stringify(param))
			.then((response) => {
				...
				if(response.data.success){
                    ...
                    if(response.data.result && response.data.result.length) {
                        ...
                        this.arr.push({value: "0", name: '低告警'});
                        this.arrm.push('低告警');
                        this.allnum = '0';
                        this.init();
                        this.loading = false;
			...
			//查询各状态的报警数量
		this.$axios.post(`BMPlatServers/warn/queryWarningStatus`,this.$qs.stringify(param))
			.then((response) => {
                console.log(response);
				if(response.data.success){
                    this.starr = [];
                    ...
                    if(response.data.result && response.data.result.length) {
                        ...
                        this.starr.push({value: 0, name: '未处理'});
                        this.starrm.push('未处理');
                        this.allnum = '0';
                        this.init();
                        this.loading = false;
                    }
