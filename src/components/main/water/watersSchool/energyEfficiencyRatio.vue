...
export default {
	data(){
		return{
			...
			pattern: 1,
			patternList: [{label: '医院', value: 1}, {label: '楼宇', value: 2},{label: '房间', value: 3}],
			types: 1,
			person:[],
			personValue:[],
			building: '',
	...
	methods:{
		...
		initData(){
			...
			console.log(data)
			this.$axios.post(`BMPlatServers/water/queryhistorycompared`, this.$qs.stringify(data)).then((res)=>{
			    console.log(res);
			    if(res.data.success) {
