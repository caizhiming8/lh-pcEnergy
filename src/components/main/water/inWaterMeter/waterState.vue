		...
		<div class="waterState">
			...
			<div class="placeTable">
			    <el-table tooltip-effect="light" :data="tableData" style="width: 100%; margin-top: 20px" :row-style="{height:0}" :cell-style="{padding:'8px 0'}" :header-cell-style="{background:'#F5F7FA',color:'#606266', height: '0px', padding: '15px 0'}">
			        ...
			        <el-table-column :show-overflow-tooltip="true" prop="ammeter_name" label="水表名称" align='center'></el-table-column>
	...
	methods:{
		initData(){
			let data = {
			 ...
			 this.$axios.post(`BMPlatServers/water/queryWaterStatus`, this.$qs.stringify(data)).then((res)=>{
			    console.log(res);
			    if(res.data.success) {
